...
export default function SahyogCases({ token, admin, root }) {
  ...
  return <>
    ...
    {isLoading ? <p className="py-12 text-center text-slate-500">Loading Sahyog cases…</p> : <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{cases.map((item) => <article key={item._id} className="rounded-xl border bg-white p-5 shadow-sm"><div className="flex items-start justify-between gap-3"><div><h3 className="font-bold text-[#5a0615]">{item.memberId?.fullName || "Member"}</h3><p className="mt-1 text-sm text-slate-500">{item.memberId?.memberId || item.sahyogId}</p></div><StatusBadge value={item.status} /></div><dl className="mt-5 space-y-2 text-sm"><div className="flex justify-between gap-3"><dt className="text-slate-500">Death date</dt><dd>{new Date(item.dateOfDeath).toLocaleDateString("en-IN")}</dd></div><div className="flex justify-between gap-3"><dt className="text-slate-500">Minimum donation</dt><dd>{item.minimumDonationAmount ? `₹${item.minimumDonationAmount}` : "Not set"}</dd></div><div className="flex justify-between gap-3"><dt className="text-slate-500">Verified donations</dt><dd>₹{item.donationSummary.amount} ({item.donationSummary.count})</dd></div></dl><div className="mt-5 flex flex-wrap gap-3 border-t pt-4"><button onClick={() => navigate(item._id)} className="font-semibold text-[#78081c]">View</button>{can(admin, "update") && <button onClick={() => navigate(`${item._id}/edit`)} className="font-semibold text-slate-700">Edit</button>}{can(admin, "delete") && <LoadingButton loading={deletingId === item._id} onClick={() => disable(item._id)} className="font-semibold text-rose-700">Disable</LoadingButton>}</div></article>)}</div>}
